var makeGLCurveFromTesselation = function(s){
...
var makeGLMeshFromTesselation = function(s){
...
verb.geom.NurbsCurve.prototype.toGlType = function(callback) {
  
  if (callback){
    this.tesselate({}, function(s){
      callback(makeGLCurveFromTesselation(s));
    })
  }

  var s = this.tesselate();
  return makeGLCurveFromTesselation(s);

};

verb.geom.NurbsSurface.prototype.toGlType = function(callback) {

  if (callback){
    this.tesselate({}, function(s){
      callback( makeGLMeshFromTesselation(s) );
    })
  }

  var s = this.tesselate();
  return makeGLMeshFromTesselation(s)
...
function buildViewer(gl, mesh){
  ...
  gl.ondraw = function() {
    ...
    if (mesh instanceof GL.Mesh){
      ...
      gl.enable(gl.BLEND);
      that.blackShader.draw( mesh, gl.LINES );
      gl.disable(gl.BLEND);
...
function Viewer(ele, width, height, depth, async) {
  ...
  // make the mesh
  ele.toGlType(function(mesh){

    attachViewer( gl, mesh );
