...
verb.Verb.main = function() {
	haxe.Log.trace("verb 2.0.0",{ fileName : "Verb.hx", lineNumber : 45, className : "verb.Verb", methodName : "main"});
};
verb.core = {};
...
verb.core.BoundingBox = $hx_exports.core.BoundingBox = function(pts) {
	this.max = null;
	this.min = null;
	this.dim = 3;
...
verb.core.BoundingBox.__name__ = ["verb","core","BoundingBox"];
verb.core.BoundingBox.intervalsOverlap = function(a1,a2,b1,b2,tol) {
	if(tol == null) tol = -1;
	var tol1;
	if(tol < -0.5) tol1 = verb.core.BoundingBox.TOLERANCE; else tol1 = tol;
...
verb.core.BoundingBox.prototype = {
	fromPoint: function(pt) {
		return new verb.core.BoundingBox([pt]);
	}
	,add: function(point) {
		if(!this.initialized) {
	...
	,contains: function(point,tol) {
		...
		return this.intersects(new verb.core.BoundingBox([point]),tol);
	}
	,intersects: function(bb,tol) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			if(!verb.core.BoundingBox.intervalsOverlap(a1[i],a2[i],b1[i],b2[i],tol)) return false;
		}
		return true;
	}
	,clear: function() {
	...
	,intersect: function(bb,tol) {
		...
		return new verb.core.BoundingBox([minbb,maxbb]);
	}
};
...
verb.core.MeshData.__name__ = ["verb","core","MeshData"];
verb.core.MeshData.empty = function() {
	return new verb.core.MeshData([],[],[],[]);
};
verb.core.PolylineData = $hx_exports.core.PolylineData = function(points,params) {
	this.points = points;
	this.params = params;
};
verb.core.PolylineData.__name__ = ["verb","core","PolylineData"];
verb.core.VolumeData = $hx_exports.core.VolumeData = function(degreeU,degreeV,degreeW,knotsU,knotsV,knotsW,controlPoints) {
	this.degreeU = degreeU;
	this.degreeV = degreeV;
...
verb.core.CurveCurveIntersection.__name__ = ["verb","core","CurveCurveIntersection"];
verb.core.CurveSurfaceIntersection = $hx_exports.core.CurveSurfaceIntersection = function(u,uv,curvePoint,surfacePoint) {
	this.u = u;
	this.uv = uv;
	this.curvePoint = curvePoint;
	this.surfacePoint = surfacePoint;
};
verb.core.CurveSurfaceIntersection.__name__ = ["verb","core","CurveSurfaceIntersection"];
...
verb.core.MeshIntersectionPoint.__name__ = ["verb","core","MeshIntersectionPoint"];
...
verb.core.PolylineMeshIntersection.__name__ = ["verb","core","PolylineMeshIntersection"];
...
verb.core.SurfaceSurfaceIntersectionPoint.__name__ = ["verb","core","SurfaceSurfaceIntersectionPoint"];
...
verb.core.TriSegmentIntersection.__name__ = ["verb","core","TriSegmentIntersection"];
verb.core.CurveTriPoint = $hx_exports.core.CurveTriPoint = function(u,point,uv) {
	this.u = u;
	this.point = point;
	this.uv = uv;
};
verb.core.CurveTriPoint.__name__ = ["verb","core","CurveTriPoint"];
verb.core.SurfacePoint = function(point,normal,uv,id,degen) {
...
verb.core.SurfacePoint.__name__ = ["verb","core","SurfacePoint"];
verb.core.SurfacePoint.fromUv = function(u,v) {
	return new verb.core.SurfacePoint(null,null,[u,v]);
};
verb.core.CurvePoint = $hx_exports.core.CurvePoint = function(u,pt) {
	this.u = u;
	this.pt = pt;
};
	...
	,nearest: function(point,maxNodes,maxDistance) {
		...
		nearestSearch1 = function(node) {
			...
			var saveNode = function(node1,distance) {
				bestNodes.push(new verb.core.Pair(node1,distance));
		...
		while(_g4 < maxNodes) {
			var i3 = _g4++;
			bestNodes.push(new verb.core.Pair(null,maxDistance));
		}
		...
		while(_g5 < maxNodes) {
			var i4 = _g5++;
			if(bestNodes.content[i4].item0 != null) result.push(new verb.core.Pair(bestNodes.content[i4].item0.kdPoint,bestNodes.content[i4].item1));
		}
		return result;
	}
};
...
verb.core.LazyCurveBoundingBoxTree.prototype = {
	...
	,boundingBox: function() {
		if(this._boundingBox == null) this._boundingBox = new verb.core.BoundingBox(verb.eval.Eval.dehomogenize1d(this._curve.controlPoints));
...
verb.core.LazyMeshBoundingBoxTree.prototype = {
	split: function() {
		...
		return new verb.core.Pair(new verb.core.LazyMeshBoundingBoxTree(this._mesh,l),new verb.core.LazyMeshBoundingBoxTree(this._mesh,r));
	}
	,boundingBox: function() {
...
verb.core.LazyPolylineBoundingBoxTree.prototype = {
	...
	,boundingBox: function() {
		if(this._boundingBox == null) this._boundingBox = new verb.core.BoundingBox(this._polyline.points);
		return this._boundingBox;
	}
	,'yield': function() {
...
verb.core.LazySurfaceBoundingBoxTree.prototype = {
	split: function() {
		...
		var max;
		if(this._splitV) {
			min = verb.core.ArrayExtensions.first(this._surface.knotsV);
			max = verb.core.ArrayExtensions.last(this._surface.knotsV);
		} else {
			min = verb.core.ArrayExtensions.first(this._surface.knotsU);
	...
	,boundingBox: function() {
		if(this._boundingBox == null) {
			this._boundingBox = new verb.core.BoundingBox();
			var _g = 0;
...
verb.core.Mesh.sortTrianglesOnLongestAxis = function(bb,mesh,faceIndices) {
	...
	while(_g < faceIndices.length) {
		...
		minCoordFaceMap.push(new verb.core.Pair(tri_min,faceIndex));
	}
...
verb.core.SurfaceBoundingBoxTree = function(surface,splitV,knotTolU,knotTolV) {
	if(splitV == null) splitV = false;
	this._boundingBox = null;
	this._surface = surface;
	if(knotTolU == null) knotTolU = verb.core.Vec.domain(surface.knotsU) / 16;
...
verb.core.SurfaceBoundingBoxTree.__interfaces__ = [verb.core.IBoundingBoxTree];
verb.core.SurfaceBoundingBoxTree.prototype = {
	split: function() {
		return this._children;
	}
	,boundingBox: function() {
...
verb.core.Vec.positiveAngleBetween = function(a,b,n) {
	...
	if(Math.abs(s) < verb.core.Constants.EPSILON) return w;
...
verb.core.Vec.domain = function(a) {
	return verb.core.ArrayExtensions.last(a) - verb.core.ArrayExtensions.first(a);
};
...
verb.core.Vec.span = function(min,max,step) {
	if(step == null) return [];
	if(step < verb.core.Constants.EPSILON) return [];
...
verb.core.Vec.sortedSetUnion = function(a,b) {
	...
	while(ai < a.length || bi < b.length) {
		...
		var diff = a[ai] - b[bi];
		if(Math.abs(diff) < verb.core.Constants.EPSILON) {
			merged.push(a[ai]);
			ai++;
			bi++;
			continue;
...
verb.core.Vec.sortedSetSub = function(a,b) {
	...
	while(ai < a.length) {
		...
		if(Math.abs(a[ai] - b[bi]) < verb.core.Constants.EPSILON) {
			ai++;
			bi++;
			continue;
		}
...
verb.eval.Analyze.isRationalSurfaceClosed = function(surface,uDir) {
	...
	while(_g1 < _g) {
		var i = _g1++;
		var test = verb.core.Vec.dist(verb.core.ArrayExtensions.first(cpts)[i],verb.core.ArrayExtensions.last(cpts)[i]) < verb.core.Constants.EPSILON;
		if(!test) return false;
	}
	return true;
};
verb.eval.Analyze.rationalSurfaceClosestPoint = function(surface,p) {
	var uv = verb.eval.Analyze.rationalSurfaceClosestParam(surface,p);
	return verb.eval.Eval.rationalSurfacePoint(surface,uv[0],uv[1]);
};
verb.eval.Analyze.rationalSurfaceClosestParam = function(surface,p) {
	var maxits = 5;
	var i = 0;
	var e;
	var eps1 = 0.0001;
	...
	var f = function(uv) {
		return verb.eval.Eval.rationalSurfaceDerivatives(surface,uv[0],uv[1],2);
	};
...
verb.eval.Analyze.rationalCurveClosestPoint = function(curve,p) {
	return verb.eval.Eval.rationalCurvePoint(curve,verb.eval.Analyze.rationalCurveClosestParam(curve,p));
};
verb.eval.Analyze.rationalCurveClosestParam = function(curve,p) {
	var min = Math.POSITIVE_INFINITY;
	var u = 0.0;
	var pts = verb.eval.Tess.rationalCurveRegularSample(curve,curve.controlPoints.length * curve.degree,true);
	var _g1 = 0;
	var _g = pts.length - 1;
	while(_g1 < _g) {
		var i = _g1++;
	...
	var cu = u;
	var f = function(u2) {
		return verb.eval.Eval.rationalCurveDerivatives(curve,u2,2);
	};
...
verb.eval.Analyze.rationalCurveParamAtArcLength = function(curve,len,tol,beziers,bezierLengths) {
	if(tol == null) tol = 1e-3;
	if(len < verb.core.Constants.EPSILON) return curve.knots[0];
	var crvs;
	if(beziers != null) crvs = beziers; else crvs = verb.eval.Modify.decomposeCurveIntoBeziers(curve);
	var i = 0;
	var cc = crvs[i];
	...
	while(cl < len && i < crvs.length) {
		if(i < bezier_lengths.length) bezier_lengths[i] = bezier_lengths[i]; else bezier_lengths[i] = verb.eval.Analyze.rationalBezierCurveArcLength(curve);
		cl += bezier_lengths[i];
		if(len < cl + verb.core.Constants.EPSILON) return verb.eval.Analyze.rationalBezierCurveParamAtArcLength(curve,len,tol,bezier_lengths[i]);
		i++;
	}
	return -1;
};
verb.eval.Analyze.rationalBezierCurveParamAtArcLength = function(curve,len,tol,totalLength) {
	if(len < 0) return curve.knots[0];
	var totalLen;
	if(totalLength != null) totalLen = totalLength; else totalLen = verb.eval.Analyze.rationalBezierCurveArcLength(curve);
	...
	while(end_l - start_l > tol1) {
		...
		mid_l = verb.eval.Analyze.rationalBezierCurveArcLength(curve,mid_p);
		if(mid_l > len) {
			end_p = mid_p;
			end_l = mid_l;
		} else {
...
verb.eval.Analyze.rationalCurveArcLength = function(curve,u,gaussDegIncrease) {
	if(gaussDegIncrease == null) gaussDegIncrease = 16;
	if(u == null) u = verb.core.ArrayExtensions.last(curve.knots); else u = u;
	var crvs = verb.eval.Modify.decomposeCurveIntoBeziers(curve);
	var i = 0;
	var cc = crvs[0];
	var sum = 0.0;
	while(i < crvs.length && cc.knots[0] + verb.core.Constants.EPSILON < u) {
		var param = Math.min(verb.core.ArrayExtensions.last(cc.knots),u);
		sum += verb.eval.Analyze.rationalBezierCurveArcLength(cc,param,gaussDegIncrease);
		cc = crvs[++i];
	}
	return sum;
};
verb.eval.Analyze.rationalBezierCurveArcLength = function(curve,u,gaussDegIncrease) {
	if(gaussDegIncrease == null) gaussDegIncrease = 16;
	var u1;
	...
	while(_g < gaussDeg) {
		var i = _g++;
		cu = z * verb.eval.Analyze.Tvalues[gaussDeg][i] + z + curve.knots[0];
		tan = verb.eval.Eval.rationalCurveDerivatives(curve,cu,1);
		sum += verb.eval.Analyze.Cvalues[gaussDeg][i] * verb.core.Vec.norm(tan[1]);
	}
	return z * sum;
};
...
verb.eval.KnotMultiplicity.__name__ = ["verb","eval","KnotMultiplicity"];
verb.eval.KnotMultiplicity.prototype = {
	inc: function() {
		this.mult++;
	}
};
verb.eval.Check = $hx_exports.eval.Check = function() { };
verb.eval.Check.__name__ = ["verb","eval","Check"];
verb.eval.Check.isValidKnotVector = function(vec,degree) {
	if(vec.length == 0) return false;
	if(vec.length < (degree + 1) * 2) return false;
	var rep = verb.core.ArrayExtensions.first(vec);
	var _g1 = 0;
	var _g = degree + 1;
	while(_g1 < _g) {
		var i = _g1++;
		if(Math.abs(vec[i] - rep) > verb.core.Constants.EPSILON) return false;
	}
	rep = verb.core.ArrayExtensions.last(vec);
	var _g11 = vec.length - degree - 1;
	var _g2 = vec.length;
	while(_g11 < _g2) {
		var i1 = _g11++;
		if(Math.abs(vec[i1] - rep) > verb.core.Constants.EPSILON) return false;
	}
	return verb.eval.Check.isNonDecreasing(vec);
};
verb.eval.Check.isNonDecreasing = function(vec) {
	var rep = verb.core.ArrayExtensions.first(vec);
	var _g1 = 0;
	var _g = vec.length;
	while(_g1 < _g) {
		var i = _g1++;
		if(vec[i] < rep - verb.core.Constants.EPSILON) return false;
		rep = vec[i];
	}
	return true;
};
...
verb.eval.Check.nurbsSurfaceData = function(data) {
	...
	if(!verb.eval.Check.isValidKnotVector(data.knotsU,data.degreeU) || !verb.eval.Check.isValidKnotVector(data.knotsV,data.degreeV)) throw "Invalid knot vector format!  Should begin with degree + 1 repeats and end with degree + 1 repeats!";
	return data;
};
...
verb.eval.Divide.rationalCurveByArcLength = function(curve,l) {
	var crvs = verb.eval.Modify.decomposeCurveIntoBeziers(curve);
...
verb.eval.Eval.volumePoint = function(volume,u,v,w) {
...
verb.eval.Eval.volumePointGivenNML = function(volume,n,m,l,u,v,w) {
	...
	var knotSpan_index_u = verb.eval.Eval.knotSpanGivenN(n,degreeU,u,knotsU);
	var knotSpan_index_v = verb.eval.Eval.knotSpanGivenN(m,degreeV,v,knotsV);
	var knotSpan_index_w = verb.eval.Eval.knotSpanGivenN(l,degreeW,w,knotsW);
	var u_basis_vals = verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(knotSpan_index_u,u,degreeU,knotsU);
	var v_basis_vals = verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(knotSpan_index_v,v,degreeV,knotsV);
	var w_basis_vals = verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(knotSpan_index_w,w,degreeW,knotsW);
...
verb.eval.Eval.rationalCurveTangent = function(curve,u) {
	var derivs = verb.eval.Eval.rationalCurveDerivatives(curve,u,1);
	return derivs[1];
};
verb.eval.Eval.rationalSurfaceNormal = function(surface,u,v) {
	var derivs = verb.eval.Eval.rationalSurfaceDerivatives(surface,u,v,1);
	return verb.core.Vec.cross(derivs[1][0],derivs[0][1]);
};
verb.eval.Eval.rationalSurfaceDerivatives = function(surface,u,v,numDerivs) {
	if(numDerivs == null) numDerivs = 1;
	var ders = verb.eval.Eval.surfaceDerivatives(surface,u,v,numDerivs);
	var Aders = verb.eval.Eval.rational2d(ders);
	var wders = verb.eval.Eval.weight2d(ders);
	var SKL = new Array();
...
verb.eval.Eval.rationalSurfacePoint = function(surface,u,v) {
	return verb.eval.Eval.dehomogenize(verb.eval.Eval.surfacePoint(surface,u,v));
};
verb.eval.Eval.rationalCurveDerivatives = function(curve,u,numDerivs) {
	if(numDerivs == null) numDerivs = 1;
	var ders = verb.eval.Eval.curveDerivatives(curve,u,numDerivs);
	var Aders = verb.eval.Eval.rational1d(ders);
	var wders = verb.eval.Eval.weight1d(ders);
	var k = 0;
	var i = 0;
	var CK = [];
	var _g1 = 0;
...
verb.eval.Eval.rationalCurvePoint = function(curve,u) {
	return verb.eval.Eval.dehomogenize(verb.eval.Eval.curvePoint(curve,u));
};
verb.eval.Eval.dehomogenize = function(homoPoint) {
...
verb.eval.Eval.rational1d = function(homoPoints) {
	var dim = homoPoints[0].length - 1;
	return homoPoints.map(function(x) {
		return x.slice(0,dim);
	});
};
verb.eval.Eval.rational2d = function(homoPoints) {
	return homoPoints.map(verb.eval.Eval.rational1d);
};
verb.eval.Eval.weight1d = function(homoPoints) {
	var dim = homoPoints[0].length - 1;
	return homoPoints.map(function(x) {
		return x[dim];
	});
};
verb.eval.Eval.weight2d = function(homoPoints) {
	return homoPoints.map(verb.eval.Eval.weight1d);
};
verb.eval.Eval.dehomogenize1d = function(homoPoints) {
	return homoPoints.map(verb.eval.Eval.dehomogenize);
};
verb.eval.Eval.dehomogenize2d = function(homoPoints) {
	return homoPoints.map(verb.eval.Eval.dehomogenize1d);
};
verb.eval.Eval.homogenize1d = function(controlPoints,weights) {
...
verb.eval.Eval.homogenize2d = function(controlPoints,weights) {
	var rows = controlPoints.length;
	var homo_controlPoints = new Array();
	var weights1;
	...
	while(_g11 < rows) {
		var i1 = _g11++;
		homo_controlPoints.push(verb.eval.Eval.homogenize1d(controlPoints[i1],weights1[i1]));
	}
	return homo_controlPoints;
};
verb.eval.Eval.surfaceDerivatives = function(surface,u,v,numDerivs) {
...
verb.eval.Eval.surfaceDerivativesGivenNM = function(n,m,surface,u,v,numDerivs) {
	var degreeU = surface.degreeU;
	var degreeV = surface.degreeV;
	var controlPoints = surface.controlPoints;
	var knotsU = surface.knotsU;
	var knotsV = surface.knotsV;
	if(!verb.eval.Eval.areValidRelations(degreeU,controlPoints.length,knotsU.length) || !verb.eval.Eval.areValidRelations(degreeV,controlPoints[0].length,knotsV.length)) throw "Invalid relations between control points, knot vector, and n";
	...
	var knotSpan_index_u = verb.eval.Eval.knotSpanGivenN(n,degreeU,u,knotsU);
	var knotSpan_index_v = verb.eval.Eval.knotSpanGivenN(m,degreeV,v,knotsV);
	var uders = verb.eval.Eval.derivativeBasisFunctionsGivenNI(knotSpan_index_u,u,degreeU,n,knotsU);
	var vders = verb.eval.Eval.derivativeBasisFunctionsGivenNI(knotSpan_index_v,v,degreeV,m,knotsV);
...
verb.eval.Eval.surfacePoint = function(surface,u,v) {
	var n = surface.knotsU.length - surface.degreeU - 2;
	var m = surface.knotsV.length - surface.degreeV - 2;
	return verb.eval.Eval.surfacePointGivenNM(n,m,surface,u,v);
};
verb.eval.Eval.surfacePointGivenNM = function(n,m,surface,u,v) {
	var degreeU = surface.degreeU;
	var degreeV = surface.degreeV;
	var controlPoints = surface.controlPoints;
	var knotsU = surface.knotsU;
...
verb.eval.Eval.curveDerivatives = function(crv,u,numDerivs) {
	var n = crv.knots.length - crv.degree - 2;
	return verb.eval.Eval.curveDerivativesGivenN(n,crv,u,numDerivs);
};
verb.eval.Eval.curveDerivativesGivenN = function(n,curve,u,numDerivs) {
	var degree = curve.degree;
	var controlPoints = curve.controlPoints;
	var knots = curve.knots;
	if(!verb.eval.Eval.areValidRelations(degree,controlPoints.length,knots.length)) throw "Invalid relations between control points, knot vector, and n";
	var dim = controlPoints[0].length;
	var du;
	if(numDerivs < degree) du = numDerivs; else du = degree;
	var CK = verb.core.Vec.zeros2d(du + 1,dim);
	var knotSpan_index = verb.eval.Eval.knotSpanGivenN(n,degree,u,knots);
	var nders = verb.eval.Eval.derivativeBasisFunctionsGivenNI(knotSpan_index,u,degree,du,knots);
	var k = 0;
	var j = 0;
	var _g1 = 0;
	var _g = du + 1;
...
verb.eval.Eval.curvePoint = function(curve,u) {
	var n = curve.knots.length - curve.degree - 2;
	return verb.eval.Eval.curvePointGivenN(n,curve,u);
};
verb.eval.Eval.areValidRelations = function(degree,num_controlPoints,knots_length) {
	return num_controlPoints + degree + 1 - knots_length == 0;
};
verb.eval.Eval.curvePointGivenN = function(n,curve,u) {
	var degree = curve.degree;
	var controlPoints = curve.controlPoints;
	var knots = curve.knots;
	if(!verb.eval.Eval.areValidRelations(degree,controlPoints.length,knots.length)) {
		throw "Invalid relations between control points, knot Array, and n";
		return null;
	}
	var knotSpan_index = verb.eval.Eval.knotSpanGivenN(n,degree,u,knots);
	var basis_values = verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(knotSpan_index,u,degree,knots);
...
verb.eval.Eval.derivativeBasisFunctions = function(u,degree,knots) {
	var knotSpan_index = verb.eval.Eval.knotSpan(degree,u,knots);
	var m = knots.length - 1;
	var n = m - degree - 1;
	return verb.eval.Eval.derivativeBasisFunctionsGivenNI(knotSpan_index,u,degree,n,knots);
};
verb.eval.Eval.derivativeBasisFunctionsGivenNI = function(knotSpan_index,u,p,n,knots) {
...
verb.eval.Eval.basisFunctions = function(u,degree,knots) {
	var knotSpan_index = verb.eval.Eval.knotSpan(degree,u,knots);
	return verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(knotSpan_index,u,degree,knots);
};
verb.eval.Eval.basisFunctionsGivenKnotSpanIndex = function(knotSpan_index,u,degree,knots) {
...
verb.eval.Eval.knotSpan = function(degree,u,knots) {
	return verb.eval.Eval.knotSpanGivenN(knots.length - degree - 2,degree,u,knots);
};
verb.eval.Eval.knotSpanGivenN = function(n,degree,u,knots) {
	if(u > knots[n + 1] - verb.core.Constants.EPSILON) return n;
	if(u < knots[degree] + verb.core.Constants.EPSILON) return degree;
	var low = degree;
	var high = n + 1;
...
verb.eval.MarchStepState = { __ename__ : true, __constructs__ : ["OutOfBounds","InsideDomain","AtBoundary","CompleteLoop","CoincidentStartPoint"] };
verb.eval.MarchStepState.OutOfBounds = ["OutOfBounds",0];
verb.eval.MarchStepState.OutOfBounds.toString = $estr;
verb.eval.MarchStepState.OutOfBounds.__enum__ = verb.eval.MarchStepState;
verb.eval.MarchStepState.InsideDomain = ["InsideDomain",1];
verb.eval.MarchStepState.InsideDomain.toString = $estr;
verb.eval.MarchStepState.InsideDomain.__enum__ = verb.eval.MarchStepState;
verb.eval.MarchStepState.AtBoundary = ["AtBoundary",2];
verb.eval.MarchStepState.AtBoundary.toString = $estr;
verb.eval.MarchStepState.AtBoundary.__enum__ = verb.eval.MarchStepState;
verb.eval.MarchStepState.CompleteLoop = ["CompleteLoop",3];
verb.eval.MarchStepState.CompleteLoop.toString = $estr;
verb.eval.MarchStepState.CompleteLoop.__enum__ = verb.eval.MarchStepState;
verb.eval.MarchStepState.CoincidentStartPoint = ["CoincidentStartPoint",4];
verb.eval.MarchStepState.CoincidentStartPoint.toString = $estr;
verb.eval.MarchStepState.CoincidentStartPoint.__enum__ = verb.eval.MarchStepState;
verb.eval.MarchStep = function(step,olduv0,olduv1,uv0,uv1,oldpoint,point,state,stepCount) {
	this.stepCount = 0;
	this.step = step;
	this.olduv0 = olduv0;
	this.olduv1 = olduv1;
...
verb.eval.MarchStep.__name__ = ["verb","eval","MarchStep"];
verb.eval.MarchStep.outOfBounds = function() {
	return new verb.eval.MarchStep(null,null,null,null,null,null,null,verb.eval.MarchStepState.OutOfBounds,0);
};
verb.eval.MarchStep.init = function(pt) {
	return new verb.eval.MarchStep(null,null,null,pt.uv0,pt.uv1,null,pt.point,verb.eval.MarchStepState.InsideDomain,0);
};
verb.eval.ExpIntersect = $hx_exports.eval.ExpIntersect = function() { };
verb.eval.ExpIntersect.__name__ = ["verb","eval","ExpIntersect"];
verb.eval.ExpIntersect.outsideDomain = function(surface,uv) {
	var u = uv[0];
	var v = uv[1];
	return u < verb.core.ArrayExtensions.first(surface.knotsU) || v < verb.core.ArrayExtensions.first(surface.knotsV) || u > verb.core.ArrayExtensions.last(surface.knotsU) || v > verb.core.ArrayExtensions.last(surface.knotsV);
};
verb.eval.ExpIntersect.clampToDomain = function(surface,uv) {
	var u = uv[0];
	var v = uv[1];
	if(u < verb.core.ArrayExtensions.first(surface.knotsU)) u = verb.core.ArrayExtensions.first(surface.knotsU);
	if(u > verb.core.ArrayExtensions.last(surface.knotsU)) u = verb.core.ArrayExtensions.last(surface.knotsU);
	if(v < verb.core.ArrayExtensions.first(surface.knotsV)) v = verb.core.ArrayExtensions.first(surface.knotsV);
	if(v > verb.core.ArrayExtensions.last(surface.knotsV)) u = verb.core.ArrayExtensions.last(surface.knotsV);
	return [u,v];
};
verb.eval.ExpIntersect.clampStep = function(surface,uv,step) {
	var u = uv[0];
	var v = uv[1];
	var nu = u + step[0];
	if(nu > verb.core.ArrayExtensions.last(surface.knotsU) + verb.core.Constants.EPSILON) step = verb.core.Vec.mul((verb.core.ArrayExtensions.last(surface.knotsU) - u) / step[0],step); else if(nu < verb.core.ArrayExtensions.first(surface.knotsU) - verb.core.Constants.EPSILON) step = verb.core.Vec.mul((verb.core.ArrayExtensions.first(surface.knotsU) - u) / step[0],step);
	var nv = v + step[1];
	if(nv > verb.core.ArrayExtensions.last(surface.knotsV) + verb.core.Constants.EPSILON) step = verb.core.Vec.mul((verb.core.ArrayExtensions.last(surface.knotsV) - v) / step[1],step); else if(nv < verb.core.ArrayExtensions.first(surface.knotsV) - verb.core.Constants.EPSILON) step = verb.core.Vec.mul((verb.core.ArrayExtensions.first(surface.knotsV) - v) / step[1],step);
	return step;
};
verb.eval.ExpIntersect.march = function(surface0,surface1,prev,currentIndex,allStartPts,tol) {
	var first = allStartPts[currentIndex];
	var uv0 = prev.uv0;
	var uv1 = prev.uv1;
	var derivs0 = verb.eval.Eval.rationalSurfaceDerivatives(surface0,uv0[0],uv0[1],1);
	var derivs1 = verb.eval.Eval.rationalSurfaceDerivatives(surface1,uv1[0],uv1[1],1);
	var p = derivs0[0][0];
	var q = derivs1[0][0];
	...
	var stepLength = verb.eval.ExpIntersect.INIT_STEP_LENGTH;
	if(prev.oldpoint != null) {
		var denom = Math.acos(verb.core.Vec.dot(verb.core.Vec.normalized(prev.step),unitStep));
		if(Math.abs(denom) < verb.core.Constants.EPSILON) stepLength = verb.eval.ExpIntersect.LINEAR_STEP_LENGTH; else {
	...
	var state = verb.eval.MarchStepState.InsideDomain;
	if(verb.eval.ExpIntersect.outsideDomain(surface0,newuv0)) {
		state = verb.eval.MarchStepState.AtBoundary;
		var l = verb.core.Vec.norm(stepuv0);
		stepuv0 = verb.eval.ExpIntersect.clampStep(surface0,uv0,stepuv0);
		stepuv1 = verb.core.Vec.mul(verb.core.Vec.norm(stepuv0) / l,stepuv1);
	}
	if(verb.eval.ExpIntersect.outsideDomain(surface1,newuv1)) {
		state = verb.eval.MarchStepState.AtBoundary;
		var l1 = verb.core.Vec.norm(stepuv1);
		stepuv1 = verb.eval.ExpIntersect.clampStep(surface1,uv1,stepuv1);
	...
	var relaxed = verb.eval.Intersect.surfacesAtPointWithEstimate(surface0,surface1,newuv0,newuv1,tol);
	if(prev.stepCount > 5 && prev.olduv0 != null && verb.core.Trig.distToSegment(prev.point,first.point,relaxed.point) < 10 * tol) return new verb.eval.MarchStep(step,prev.uv0,prev.uv1,first.uv0,first.uv1,prev.point,first.point,verb.eval.MarchStepState.CompleteLoop,prev.stepCount + 1);
	if(prev.stepCount > 5 && verb.eval.ExpIntersect.isCoincidentWithStartPoint(relaxed.point,currentIndex,allStartPts,10 * tol)) state = verb.eval.MarchStepState.CoincidentStartPoint;
	return new verb.eval.MarchStep(step,prev.uv0,prev.uv1,relaxed.uv0,relaxed.uv1,prev.point,relaxed.point,state,prev.stepCount + 1);
};
verb.eval.ExpIntersect.isCoincidentWithStartPoint = function(point,currentIndex,allStartPts,tol) {
	if(currentIndex == 0) return false;
	var _g = 0;
	while(_g < currentIndex) {
		var i = _g++;
		var dist = verb.core.Vec.distSquared(allStartPts[i].point,allStartPts[currentIndex].point);
		if(dist < tol) {
			haxe.Log.trace("Coincident start point!",{ fileName : "ExpIntersect.hx", lineNumber : 257, className : "verb.eval.ExpIntersect", methodName : "isCoincidentWithStartPoint"});
			return true;
		}
	}
	return false;
};
verb.eval.ExpIntersect.completeMarch = function(surface0,surface1,startIndex,allStartPts,tol) {
	var start = allStartPts[startIndex];
	var step = verb.eval.ExpIntersect.march(surface0,surface1,verb.eval.MarchStep.init(start),startIndex,allStartPts,tol);
	if(step.state == verb.eval.MarchStepState.AtBoundary || step.state == verb.eval.MarchStepState.CoincidentStartPoint) return null;
	var $final = [];
	$final.push(start);
	while(step.state != verb.eval.MarchStepState.CoincidentStartPoint && step.state != verb.eval.MarchStepState.AtBoundary && step.state != verb.eval.MarchStepState.CompleteLoop) {
		$final.push(new verb.core.SurfaceSurfaceIntersectionPoint(step.uv0,step.uv1,step.point,-1));
		if(step.state == verb.eval.MarchStepState.CoincidentStartPoint) return null;
		step = verb.eval.ExpIntersect.march(surface0,surface1,step,startIndex,allStartPts,tol);
	}
	$final.push(new verb.core.SurfaceSurfaceIntersectionPoint(step.uv0,step.uv1,step.point,-1));
	return $final;
};
verb.eval.ExpIntersect.surfaces = function(surface0,surface1,tol) {
	var $final = [];
	var startPts = verb.eval.ExpIntersect.intersectBoundaryCurves(surface0,surface1,tol);
	var approxInner = verb.eval.ExpIntersect.approxInnerCriticalPts(surface0,surface1);
	var refinedInner = verb.eval.ExpIntersect.refineInnerCriticalPts(surface0,surface1,approxInner,tol);
	var b = true;
	var _g = 0;
	while(_g < refinedInner.length) {
		var pair = refinedInner[_g];
		++_g;
		var res = verb.eval.Intersect.curveAndSurface(verb.eval.Make.surfaceIsocurve(surface0,pair.item0[0],false),surface1);
		if(res.length == 0) continue;
		if(!b) continue;
		b = false;
		var $int = new verb.core.SurfaceSurfaceIntersectionPoint([pair.item0[0],res[0].u],res[0].uv,res[0].curvePoint,-1);
		startPts.push($int);
	}
	var i = 0;
	while(i < startPts.length) {
		haxe.Log.trace("starting at",{ fileName : "ExpIntersect.hx", lineNumber : 330, className : "verb.eval.ExpIntersect", methodName : "surfaces", customParams : [startPts[i].point]});
		var res1 = verb.eval.ExpIntersect.completeMarch(surface0,surface1,i,startPts,tol);
		if(res1 != null) {
			$final.push(res1);
			var x = verb.core.ArrayExtensions.last(res1);
			startPts.splice(i,0,x);
			haxe.Log.trace("inserting",{ fileName : "ExpIntersect.hx", lineNumber : 338, className : "verb.eval.ExpIntersect", methodName : "surfaces", customParams : [verb.core.ArrayExtensions.last(res1).point]});
			i += 2;
		}
		i++;
	}
	...
	while(_g11 < $final.length) {
		var pts = $final[_g11];
		++_g11;
		_g1.push(verb.eval.Make.rationalInterpCurve(pts.map(function(x1) {
			return x1.point;
		})));
	}
	return _g1;
};
verb.eval.ExpIntersect.refineInnerCriticalPts = function(surface0,surface1,approx,tol) {
	return approx.map(function(x) {
		return verb.eval.ExpIntersect.refineCriticalPt(surface0,surface1,x,tol);
	});
};
verb.eval.ExpIntersect.refineCriticalPt = function(surface0,surface1,approx,tol) {
	var obj = function(x) {
		var d0 = verb.eval.Eval.rationalSurfaceDerivatives(surface0,x[0],x[1],1);
		var d1 = verb.eval.Eval.rationalSurfaceDerivatives(surface1,x[2],x[3],1);
...
verb.eval.ExpIntersect.verifyInnerCriticalPts = function(surface0,surface1,approx) {
	return null;
};
verb.eval.ExpIntersect.boundingBoxLeaves = function(division) {
	if(division.indivisible(0)) return [division["yield"]()];
	var halves = division.split();
	return verb.eval.ExpIntersect.boundingBoxLeaves(halves.item0).concat(verb.eval.ExpIntersect.boundingBoxLeaves(halves.item1));
};
verb.eval.ExpIntersect.approxInnerCriticalPts = function(surface0,surface1) {
	var div0 = new verb.core.LazySurfaceBoundingBoxTree(surface0,false,0.6,0.6);
	var div1 = new verb.core.LazySurfaceBoundingBoxTree(surface1,false,0.6,0.6);
	var res = verb.eval.Intersect.boundingBoxTrees(div0,div1,0);
	var numSamples = 4;
	var criticalPts = [];
	var _g = 0;
	while(_g < res.length) {
		var srfpair = res[_g];
		++_g;
		var a = verb.eval.ExpIntersect.approxSurfaceDelPhiField(srfpair.item0,srfpair.item1,numSamples,numSamples);
		var f = a.delphi;
		var uvs0 = a.uvs0;
		var uvs1 = a.uvs1;
		var _g2 = 1;
		...
		while(_g2 < _g1) {
			...
			while(_g4 < _g3) {
				var j = _g4++;
				var num = verb.eval.ExpIntersect.approxRotationNumber([f[i][j - 1],f[i][j],f[i - 1][j],f[i - 1][j - 1]]);
				if(num != 0) {
					...
					criticalPts.push(new verb.core.Pair([midU0,midV0],[midU1,midV1]));
				}
			}
		}
	}
...
verb.eval.ExpIntersect.approxRotationNumber = function(vs) {
	var sum = 0.0;
	var l = vs.length;
	var _g1 = 1;
	var _g = l + 1;
...
verb.eval.ExpIntersect.approxSurfaceDelPhiField = function(surface0,surface1,divs_u,divs_v) {
	var tess0 = verb.eval.ExpIntersect.sampleSurfaceRegular(surface0,divs_u,divs_v);
	var tess1 = verb.eval.ExpIntersect.sampleSurfaceRegular(surface1,divs_u,divs_v);
	var minuvs = [];
	var _g1 = 0;
	...
	while(_g11 < _g8) {
		...
		while(_g31 < _g21) {
			...
			var derivs0 = verb.eval.Eval.rationalSurfaceDerivatives(surface0,uv0[0],uv0[1]);
			var derivs1 = verb.eval.Eval.rationalSurfaceDerivatives(surface1,uv1[0],uv1[1]);
...
verb.eval.ExpIntersect.sampleSurfaceRegular = function(surface,divs_u,divs_v) {
	...
	var knotsV = surface.knotsV;
	var offset = 0.005;
	var u_span = verb.core.ArrayExtensions.last(knotsU) - verb.core.ArrayExtensions.first(knotsU) - 2 * offset;
	var v_span = verb.core.ArrayExtensions.last(knotsV) - verb.core.ArrayExtensions.first(knotsV) - 2 * offset;
	var minu = verb.core.ArrayExtensions.first(knotsU) + offset;
	var minv = verb.core.ArrayExtensions.first(knotsV) + offset;
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			pointsrow.push(verb.eval.Eval.rationalSurfacePoint(surface,pt_u,pt_v));
		}
	}
	return { uvs : uvs, points : points};
};
verb.eval.ExpIntersect.intersectBoundaryCurves = function(surface0,surface1,tol) {
	var srf0bs = verb.eval.Make.surfaceBoundaryCurves(surface0);
	var srf1bs = verb.eval.Make.surfaceBoundaryCurves(surface1);
	var ints = [];
	var _g1 = 0;
	...
	while(_g1 < _g) {
		var i = _g1++;
		var crv = srf0bs[i];
		var res = verb.eval.Intersect.curveAndSurface(crv,surface1,tol);
		var _g2 = 0;
		while(_g2 < res.length) {
			...
			switch(i) {
			case 0:
				uv = [verb.core.ArrayExtensions.first(surface0.knotsU),$int.u];
				break;
			case 1:
				uv = [verb.core.ArrayExtensions.last(surface0.knotsU),$int.u];
				break;
			case 2:
				uv = [$int.u,verb.core.ArrayExtensions.first(surface0.knotsV)];
				break;
			default:
				uv = [$int.u,verb.core.ArrayExtensions.last(surface0.knotsV)];
			}
			var dist = verb.core.Vec.dist($int.curvePoint,$int.surfacePoint);
			ints.push(new verb.core.SurfaceSurfaceIntersectionPoint(uv,$int.uv,$int.curvePoint,dist));
		}
	}
	var _g11 = 0;
	...
	while(_g11 < _g3) {
		var i1 = _g11++;
		var crv1 = srf1bs[i1];
		var res1 = verb.eval.Intersect.curveAndSurface(crv1,surface0,tol);
		var _g21 = 0;
		while(_g21 < res1.length) {
			...
			switch(i1) {
			case 0:
				uv1 = [verb.core.ArrayExtensions.first(surface1.knotsU),int1.u];
				break;
			case 1:
				uv1 = [verb.core.ArrayExtensions.last(surface1.knotsU),int1.u];
				break;
			case 2:
				uv1 = [int1.u,verb.core.ArrayExtensions.first(surface1.knotsV)];
				break;
			default:
				uv1 = [int1.u,verb.core.ArrayExtensions.last(surface1.knotsV)];
			}
			var dist1 = verb.core.Vec.dist(int1.curvePoint,int1.surfacePoint);
			ints.push(new verb.core.SurfaceSurfaceIntersectionPoint(int1.uv,uv1,int1.curvePoint,dist1));
		}
	}
...
verb.eval.Intersect = $hx_exports.eval.Intersect = function() { };
verb.eval.Intersect.__name__ = ["verb","eval","Intersect"];
verb.eval.Intersect.meshSlices = function(mesh,min,max,step) {
	var bbtree = new verb.core.MeshBoundingBoxTree(mesh);
	...
	while(_g < span.length) {
		...
		var plane = new verb.core.MeshData(faces,pts,null,uvs);
		slices.push(verb.eval.Intersect.meshes(mesh,plane,bbtree));
		z += 1.0;
	}
	return slices;
};
verb.eval.Intersect.surfaces = function(surface0,surface1,tol) {
...
verb.eval.Intersect.surfacesAtPointWithEstimate = function(surface0,surface1,uv1,uv2,tol) {
	var pds;
	var p;
	var pn;
	var pu;
	...
	var its = 0;
	do {
		pds = verb.eval.Eval.rationalSurfaceDerivatives(surface0,uv1[0],uv1[1],1);
		p = pds[0][0];
		pu = pds[1][0];
		pv = pds[0][1];
		pn = verb.core.Vec.normalized(verb.core.Vec.cross(pu,pv));
		pd = verb.core.Vec.dot(pn,p);
		qds = verb.eval.Eval.rationalSurfaceDerivatives(surface1,uv2[0],uv2[1],1);
		q = qds[0][0];
		qu = qds[1][0];
		qv = qds[0][1];
		...
		var x = verb.eval.Intersect.threePlanes(pn,pd,qn,qd,fn,fd);
...
verb.eval.Intersect.meshes = function(mesh0,mesh1,bbtree0,bbtree1) {
	if(bbtree0 == null) bbtree0 = new verb.core.LazyMeshBoundingBoxTree(mesh0);
	if(bbtree1 == null) bbtree1 = new verb.core.LazyMeshBoundingBoxTree(mesh1);
	var bbints = verb.eval.Intersect.boundingBoxTrees(bbtree0,bbtree1,0);
...
verb.eval.Intersect.makeMeshIntersectionPolylines = function(segments) {
	...
	var tree = verb.eval.Intersect.kdTreeFromSegments(segments);
	var ends = [];
	var _g1 = 0;
	...
	while(_g2 < ends.length) {
		...
		var adjEnd = verb.eval.Intersect.lookupAdjacentSegment(segEnd,tree,segments.length);
...
verb.eval.Intersect.kdTreeFromSegments = function(segments) {
	var treePoints = [];
	var _g = 0;
	while(_g < segments.length) {
		var seg = segments[_g];
...
verb.eval.Intersect.lookupAdjacentSegment = function(segEnd,tree,numResults) {
...
verb.eval.Intersect.curveAndSurface = function(curve,surface,tol,crvBbTree,srfBbTree) {
	if(tol == null) tol = 1e-3;
	if(crvBbTree != null) crvBbTree = crvBbTree; else crvBbTree = new verb.core.LazyCurveBoundingBoxTree(curve);
	if(srfBbTree != null) srfBbTree = srfBbTree; else srfBbTree = new verb.core.LazySurfaceBoundingBoxTree(surface);
	var ints = verb.eval.Intersect.boundingBoxTrees(crvBbTree,srfBbTree,tol);
	return verb.core.ArrayExtensions.unique(ints.map(function(inter) {
		var crvSeg = inter.item0;
		var srfPart = inter.item1;
		var min = verb.core.ArrayExtensions.first(crvSeg.knots);
		var max = verb.core.ArrayExtensions.last(crvSeg.knots);
		var u = (min + max) / 2.0;
		var minu = verb.core.ArrayExtensions.first(srfPart.knotsU);
		var maxu = verb.core.ArrayExtensions.last(srfPart.knotsU);
		var minv = verb.core.ArrayExtensions.first(srfPart.knotsV);
		var maxv = verb.core.ArrayExtensions.last(srfPart.knotsV);
		var uv = [(minu + maxu) / 2.0,(minv + maxv) / 2.0];
		return verb.eval.Intersect.curveAndSurfaceWithEstimate(crvSeg,srfPart,[u].concat(uv),tol);
	}).filter(function(x) {
...
verb.eval.Intersect.curveAndSurfaceWithEstimate = function(curve,surface,start_params,tol) {
	if(tol == null) tol = 1e-3;
	var objective = function(x) {
		var p1 = verb.eval.Eval.rationalCurvePoint(curve,x[0]);
		var p2 = verb.eval.Eval.rationalSurfacePoint(surface,x[1],x[2]);
		var p1_p2 = verb.core.Vec.sub(p1,p2);
		return verb.core.Vec.dot(p1_p2,p1_p2);
	};
	var grad = function(x1) {
		var dc = verb.eval.Eval.rationalCurveDerivatives(curve,x1[0],1);
		var ds = verb.eval.Eval.rationalSurfaceDerivatives(surface,x1[1],x1[2],1);
...
verb.eval.Intersect.polylineAndMesh = function(polyline,mesh,tol) {
	var res = verb.eval.Intersect.boundingBoxTrees(new verb.core.LazyPolylineBoundingBoxTree(polyline),new verb.core.LazyMeshBoundingBoxTree(mesh),tol);
	var finalResults = [];
	var _g = 0;
	while(_g < res.length) {
		var event = res[_g];
		...
		var inter = verb.eval.Intersect.segmentWithTriangle(polyline.points[polid],polyline.points[polid + 1],mesh.points,mesh.faces[faceid]);
...
verb.eval.Intersect.boundingBoxTrees = function(ai,bi,tol) {
	...
	while(atrees.length > 0) {
		...
		if(ai1 && bi1) {
			results.push(new verb.core.Pair(a["yield"](),b["yield"]()));
			continue;
		} else if(ai1 && !bi1) {
			var bs = b.split();
			atrees.push(a);
...
verb.eval.Intersect.curves = function(curve1,curve2,tolerance) {
	var ints = verb.eval.Intersect.boundingBoxTrees(new verb.core.LazyCurveBoundingBoxTree(curve1),new verb.core.LazyCurveBoundingBoxTree(curve2),0);
	return verb.core.ArrayExtensions.unique(ints.map(function(x) {
		return verb.eval.Intersect.curvesWithEstimate(curve1,curve2,verb.core.ArrayExtensions.first(x.item0.knots),verb.core.ArrayExtensions.first(x.item1.knots),tolerance);
	}).filter(function(x1) {
...
verb.eval.Intersect.curvesWithEstimate = function(curve0,curve1,u0,u1,tolerance) {
	var objective = function(x) {
		var p1 = verb.eval.Eval.rationalCurvePoint(curve0,x[0]);
		var p2 = verb.eval.Eval.rationalCurvePoint(curve1,x[1]);
		var p1_p2 = verb.core.Vec.sub(p1,p2);
		return verb.core.Vec.dot(p1_p2,p1_p2);
	};
	var grad = function(x1) {
		var dc0 = verb.eval.Eval.rationalCurveDerivatives(curve0,x1[0],1);
		var dc1 = verb.eval.Eval.rationalCurveDerivatives(curve1,x1[1],1);
	...
	var p21 = verb.eval.Eval.rationalCurvePoint(curve1,u2);
	return new verb.core.CurveCurveIntersection(p11,p21,u11,u2);
};
verb.eval.Intersect.triangles = function(mesh0,faceIndex0,mesh1,faceIndex1) {
	...
	var ray = verb.eval.Intersect.planes(o0,n0,o1,n1);
	if(ray == null) return null;
	var clip1 = verb.eval.Intersect.clipRayInCoplanarTriangle(ray,mesh0,faceIndex0);
	if(clip1 == null) return null;
	var clip2 = verb.eval.Intersect.clipRayInCoplanarTriangle(ray,mesh1,faceIndex1);
	if(clip2 == null) return null;
	var merged = verb.eval.Intersect.mergeTriangleClipIntervals(clip1,clip2,mesh0,faceIndex0,mesh1,faceIndex1);
...
verb.eval.Intersect.clipRayInCoplanarTriangle = function(ray,mesh,faceIndex) {
	var tri = mesh.faces[faceIndex];
	...
	while(_g < 3) {
		...
		var o0 = o[i];
		var d0 = d[i];
		var res = verb.eval.Intersect.rays(o0,d0,ray.origin,ray.dir);
...
verb.eval.Intersect.mergeTriangleClipIntervals = function(clip1,clip2,mesh1,faceIndex1,mesh2,faceIndex2) {
	if(clip2.min.u > clip1.max.u + verb.core.Constants.EPSILON || clip1.min.u > clip2.max.u + verb.core.Constants.EPSILON) return null;
	var min;
	if(clip1.min.u > clip2.min.u) min = new verb.core.Pair(clip1.min,0); else min = new verb.core.Pair(clip2.min,1);
	var max;
	if(clip1.max.u < clip2.max.u) max = new verb.core.Pair(clip1.max,0); else max = new verb.core.Pair(clip2.max,1);
...
verb.eval.Intersect.planes = function(origin0,normal0,origin1,normal1) {
...
verb.eval.Intersect.threePlanes = function(n0,d0,n1,d1,n2,d2) {
...
verb.eval.Intersect.polylines = function(polyline0,polyline1,tol) {
	var res = verb.eval.Intersect.boundingBoxTrees(new verb.core.LazyPolylineBoundingBoxTree(polyline0),new verb.core.LazyPolylineBoundingBoxTree(polyline1),tol);
	var finalResults = [];
	var _g = 0;
	while(_g < res.length) {
		var event = res[_g];
		...
		var inter = verb.eval.Intersect.segments(polyline0.points[polid0],polyline0.points[polid0 + 1],polyline1.points[polid1],polyline1.points[polid1 + 1],tol);
		if(inter == null) continue;
...
verb.eval.Intersect.segments = function(a0,a1,b0,b1,tol) {
	...
	var int_params = verb.eval.Intersect.rays(a0,a,b0,b);
	if(int_params != null) {
...
verb.eval.Intersect.rays = function(a0,a,b0,b) {
...
verb.eval.Intersect.segmentWithTriangle = function(p0,p1,points,tri) {
	var v0 = points[tri[0]];
	var v1 = points[tri[1]];
	var v2 = points[tri[2]];
	var u = verb.core.Vec.sub(v1,v0);
...
verb.eval.Intersect.segmentAndPlane = function(p0,p1,v0,n) {
...
verb.eval.Make.rationalTranslationalSurface = function(profile,rail) {
	var pt0 = verb.eval.Eval.rationalCurvePoint(rail,verb.core.ArrayExtensions.first(rail.knots));
	var startu = verb.core.ArrayExtensions.first(rail.knots);
	...
	while(_g < numSamples) {
		var i = _g++;
		var pt = verb.core.Vec.sub(verb.eval.Eval.rationalCurvePoint(rail,startu + i * span),pt0);
		var crv = verb.eval.Modify.rationalCurveTransform(profile,[[1,0,0,pt[0]],[0,1,0,pt[1]],[0,0,1,pt[2]],[0,0,0,1]]);
		crvs.push(crv);
	}
	return verb.eval.Make.loftedSurface(crvs);
};
verb.eval.Make.surfaceBoundaryCurves = function(surface) {
	var crvs = [];
	var c0 = verb.eval.Make.surfaceIsocurve(surface,verb.core.ArrayExtensions.first(surface.knotsU),false);
	var c1 = verb.eval.Make.surfaceIsocurve(surface,verb.core.ArrayExtensions.last(surface.knotsU),false);
	var c2 = verb.eval.Make.surfaceIsocurve(surface,verb.core.ArrayExtensions.first(surface.knotsV),true);
	var c3 = verb.eval.Make.surfaceIsocurve(surface,verb.core.ArrayExtensions.last(surface.knotsV),true);
	return [c0,c1,c2,c3];
};
verb.eval.Make.surfaceIsocurve = function(surface,u,useV) {
	...
	var knotMults = verb.eval.Analyze.knotMultiplicities(knots);
	var reqKnotIndex = -1;
	var _g1 = 0;
	var _g = knotMults.length;
	while(_g1 < _g) {
		var i = _g1++;
		if(Math.abs(u - knotMults[i].knot) < verb.core.Constants.EPSILON) {
			reqKnotIndex = i;
			break;
		}
	}
	...
	var newSrf;
	if(numKnotsToInsert > 0) newSrf = verb.eval.Modify.surfaceKnotRefine(surface,verb.core.Vec.rep(numKnotsToInsert,u),useV); else newSrf = surface;
	var span = verb.eval.Eval.knotSpan(degree,u,knots);
	if(Math.abs(u - verb.core.ArrayExtensions.first(knots)) < verb.core.Constants.EPSILON) span = 0; else if(Math.abs(u - verb.core.ArrayExtensions.last(knots)) < verb.core.Constants.EPSILON) span = (useV?newSrf.controlPoints[0].length:newSrf.controlPoints.length) - 1;
	if(useV) return new verb.core.NurbsCurveData(newSrf.degreeU,newSrf.knotsU,(function($this) {
		var $r;
		var _g2 = [];
		{
			var _g11 = 0;
	...
	return new verb.core.NurbsCurveData(newSrf.degreeV,newSrf.knotsV,newSrf.controlPoints[span]);
};
verb.eval.Make.loftedSurface = function(curves,degreeV) {
	...
	while(_g1 < _g) {
		...
		var c = verb.eval.Make.rationalInterpCurve(points,degreeV,true);
		controlPoints.push(c.controlPoints);
		knotsV = c.knots;
	}
	return new verb.core.NurbsSurfaceData(degreeU,degreeV,knotsU,knotsV,controlPoints);
};
verb.eval.Make.clonedCurve = function(curve) {
	return new verb.core.NurbsCurveData(curve.degree,curve.knots.slice(),curve.controlPoints.map(function(x) {
		return x.slice();
	}));
};
verb.eval.Make.rationalBezierCurve = function(controlPoints,weights) {
	...
	return new verb.core.NurbsCurveData(degree,knots,verb.eval.Eval.homogenize1d(controlPoints,weights));
};
verb.eval.Make.fourPointSurface = function(p1,p2,p3,p4,degree) {
	...
	return new verb.core.NurbsSurfaceData(degree,degree,zeros.concat(ones),zeros.concat(ones),pts);
};
verb.eval.Make.ellipseArc = function(center,xaxis,yaxis,startAngle,endAngle) {
	...
	while(_g1 < _g) {
		...
		var inters = verb.eval.Intersect.rays(P0,verb.core.Vec.mul(1 / verb.core.Vec.norm(T0),T0),P2,verb.core.Vec.mul(1 / verb.core.Vec.norm(T2),T2));
...
verb.eval.Make.arc = function(center,xaxis,yaxis,radius,startAngle,endAngle) {
...
verb.eval.Make.polyline = function(pts) {
	var knots = [0.0,0.0];
	var lsum = 0.0;
	var _g1 = 0;
	var _g = pts.length - 1;
	...
	weights = _g2;
	return new verb.core.NurbsCurveData(1,knots,verb.eval.Eval.homogenize1d(pts.slice(0),weights));
};
verb.eval.Make.extrudedSurface = function(axis,length,profile) {
	var controlPoints = [[],[],[]];
	var weights = [[],[],[]];
	var prof_controlPoints = verb.eval.Eval.dehomogenize1d(profile.controlPoints);
	var prof_weights = verb.eval.Eval.weight1d(profile.controlPoints);
	...
	return new verb.core.NurbsSurfaceData(2,profile.degree,[0,0,0,1,1,1],profile.knots,verb.eval.Eval.homogenize2d(controlPoints,weights));
};
verb.eval.Make.cylindricalSurface = function(axis,xaxis,base,height,radius) {
	var yaxis = verb.core.Vec.cross(axis,xaxis);
	var angle = 2.0 * Math.PI;
	var circ = verb.eval.Make.arc(base,xaxis,yaxis,radius,0.0,2 * Math.PI);
	return verb.eval.Make.extrudedSurface(axis,height,circ);
};
verb.eval.Make.revolvedSurface = function(profile,center,axis,theta) {
	var prof_controlPoints = verb.eval.Eval.dehomogenize1d(profile.controlPoints);
	var prof_weights = verb.eval.Eval.weight1d(profile.controlPoints);
	var narcs;
	var knotsU;
...
verb.eval.Make.sphericalSurface = function(center,axis,xaxis,radius) {
	var arc = verb.eval.Make.arc(center,verb.core.Vec.mul(-1.0,axis),xaxis,radius,0.0,Math.PI);
	return verb.eval.Make.revolvedSurface(arc,center,axis,2 * Math.PI);
};
verb.eval.Make.conicalSurface = function(axis,xaxis,base,height,radius) {
	var angle = 2 * Math.PI;
	var prof_degree = 1;
	...
	var prof = new verb.core.NurbsCurveData(prof_degree,prof_knots,verb.eval.Eval.homogenize1d(prof_ctrl_pts,prof_weights));
	return verb.eval.Make.revolvedSurface(prof,base,axis,angle);
};
verb.eval.Make.rationalInterpCurve = function(points,degree,homogeneousPoints,start_tangent,end_tangent) {
	if(homogeneousPoints == null) homogeneousPoints = false;
	if(degree == null) degree = 3;
	if(points.length < degree + 1) throw "You need to supply at least degree + 1 points! You only supplied " + points.length + " points.";
	var us = [0.0];
	var _g1 = 1;
	...
	while(_g4 < us.length) {
		var u = us[_g4];
		++_g4;
		var span = verb.eval.Eval.knotSpanGivenN(n,degree,u,knots);
		var basisFuncs = verb.eval.Eval.basisFunctionsGivenKnotSpanIndex(span,u,degree,knots);
		var ls = span - degree;
	...
	while(_g5 < dim) {
		...
		if(!hasTangents) b = points.map((function(i3) {
			...
			b.push(verb.core.ArrayExtensions.last(points)[i3[0]]);
		}
	...
	return new verb.core.NurbsCurveData(degree,knots,controlPts);
};
verb.eval.Modify = $hx_exports.eval.Modify = function() { };
verb.eval.Modify.__name__ = ["verb","eval","Modify"];
verb.eval.Modify.curveReverse = function(curve) {
	return new verb.core.NurbsCurveData(curve.degree,verb.eval.Modify.knotsReverse(curve.knots),verb.core.ArrayExtensions.reversed(curve.controlPoints));
};
verb.eval.Modify.surfaceReverse = function(surface,useV) {
	if(useV == null) useV = false;
	if(useV) return new verb.core.NurbsSurfaceData(surface.degreeU,surface.degreeV,surface.knotsU,verb.eval.Modify.knotsReverse(surface.knotsV),(function($this) {
		var $r;
		var _g = [];
		{
			var _g1 = 0;
	...
	return new verb.core.NurbsSurfaceData(surface.degreeU,surface.degreeV,verb.eval.Modify.knotsReverse(surface.knotsU),surface.knotsV,verb.core.ArrayExtensions.reversed(surface.controlPoints));
};
verb.eval.Modify.knotsReverse = function(knots) {
	var min = verb.core.ArrayExtensions.first(knots);
	var max = verb.core.ArrayExtensions.last(knots);
	var l = [min];
...
verb.eval.Modify.unifyCurveKnotVectors = function(curves) {
	curves = curves.map(verb.eval.Make.clonedCurve);
	...
	while(_g1 < _g) {
		var i = _g1++;
		if(curves[i].degree < maxDegree) curves[i] = verb.eval.Modify.curveElevateDegree(curves[i],maxDegree);
	}
	var knotIntervals;
	var _g2 = [];
	var _g11 = 0;
	while(_g11 < curves.length) {
		var c = curves[_g11];
		++_g11;
		_g2.push(new verb.core.Interval(verb.core.ArrayExtensions.first(c.knots),verb.core.ArrayExtensions.last(c.knots)));
	}
	knotIntervals = _g2;
	var _g21 = 0;
	...
	while(_g23 < _g14) {
		...
		curves[i3] = verb.eval.Modify.curveKnotRefine(curves[i3],rem);
	}
	return curves;
};
verb.eval.Modify.imin = function(a,b) {
	if(a < b) return a; else return b;
};
verb.eval.Modify.imax = function(a,b) {
	if(a > b) return a; else return b;
};
verb.eval.Modify.curveElevateDegree = function(curve,finalDegree) {
	...
	nh = mh - ph - 1;
	return new verb.core.NurbsCurveData(finalDegree,Uh,Qw);
};
verb.eval.Modify.rationalSurfaceTransform = function(surface,mat) {
	var pts = verb.eval.Eval.dehomogenize2d(surface.controlPoints);
	var _g1 = 0;
	var _g = pts.length;
	while(_g1 < _g) {
		var i = _g1++;
	...
	return new verb.core.NurbsSurfaceData(surface.degreeU,surface.degreeV,surface.knotsU.slice(),surface.knotsV.slice(),verb.eval.Eval.homogenize2d(pts,verb.eval.Eval.weight2d(surface.controlPoints)));
};
verb.eval.Modify.rationalCurveTransform = function(curve,mat) {
	var pts = verb.eval.Eval.dehomogenize1d(curve.controlPoints);
	var _g1 = 0;
	var _g = pts.length;
	while(_g1 < _g) {
		var i = _g1++;
	...
	return new verb.core.NurbsCurveData(curve.degree,curve.knots.slice(),verb.eval.Eval.homogenize1d(pts,verb.eval.Eval.weight1d(curve.controlPoints)));
};
verb.eval.Modify.surfaceKnotRefine = function(surface,knotsToInsert,useV) {
	var newPts = [];
	var knots;
	var degree;
	var ctrlPts;
...
verb.eval.Modify.decomposeCurveIntoBeziers = function(curve) {
	...
	while(_g < knotmults.length) {
		...
		if(knotmult.mult < reqMult) {
			...
			var res = verb.eval.Modify.curveKnotRefine(new verb.core.NurbsCurveData(degree,knots,controlPoints),knotsInsert);
			knots = res.knots;
...
verb.eval.Modify.curveKnotRefine = function(curve,knotsToInsert) {
	...
	var a = verb.eval.Eval.knotSpan(degree,knotsToInsert[0],knots);
	var b = verb.eval.Eval.knotSpan(degree,knotsToInsert[r],knots);
	...
	while(j >= 0) {
		...
		while(_g14 < _g5) {
			...
			if(Math.abs(alfa) < verb.core.Constants.EPSILON) controlPoints_post[ind - 1] = controlPoints_post[ind]; else {
...
verb.eval.Modify.curveKnotInsert = function(curve,u,r) {
	var degree = curve.degree;
	var controlPoints = curve.controlPoints;
	var knots = curve.knots;
	var s = 0;
	var num_pts = controlPoints.length;
	var k = verb.eval.Eval.knotSpan(degree,u,knots);
...
verb.eval.Tess.__name__ = ["verb","eval","Tess"];
verb.eval.Tess.rationalCurveRegularSample = function(curve,numSamples,includeU) {
	return verb.eval.Tess.rationalCurveRegularSampleRange(curve,curve.knots[0],verb.core.ArrayExtensions.last(curve.knots),numSamples,includeU);
};
verb.eval.Tess.rationalCurveRegularSampleRange = function(curve,start,end,numSamples,includeU) {
	...
	while(_g < numSamples) {
		var i = _g++;
		u = start + span * i;
		if(includeU) p.push([u].concat(verb.eval.Eval.rationalCurvePoint(curve,u))); else p.push(verb.eval.Eval.rationalCurvePoint(curve,u));
	}
	return p;
};
verb.eval.Tess.rationalCurveAdaptiveSample = function(curve,tol,includeU) {
	if(includeU == null) includeU = false;
	if(tol == null) tol = 1e-6;
	if(curve.degree == 1) {
		if(!includeU) return curve.controlPoints.map(verb.eval.Eval.dehomogenize); else {
			var _g = [];
			var _g2 = 0;
			var _g1 = curve.controlPoints.length;
			while(_g2 < _g1) {
				var i = _g2++;
				_g.push([curve.knots[i + 1]].concat(verb.eval.Eval.dehomogenize(curve.controlPoints[i])));
			}
			return _g;
		}
	}
	return verb.eval.Tess.rationalCurveAdaptiveSampleRange(curve,curve.knots[0],verb.core.ArrayExtensions.last(curve.knots),tol,includeU);
};
verb.eval.Tess.rationalCurveAdaptiveSampleRange = function(curve,start,end,tol,includeU) {
	var p1 = verb.eval.Eval.rationalCurvePoint(curve,start);
	var p3 = verb.eval.Eval.rationalCurvePoint(curve,end);
	var t = 0.5 + 0.2 * Math.random();
	var mid = start + (end - start) * t;
	var p2 = verb.eval.Eval.rationalCurvePoint(curve,mid);
	...
	if(verb.core.Vec.dot(diff,diff) < tol && verb.core.Vec.dot(diff2,diff2) > tol || !verb.core.Trig.threePointsAreFlat(p1,p2,p3,tol)) {
		...
		var left_pts = verb.eval.Tess.rationalCurveAdaptiveSampleRange(curve,start,exact_mid,tol,includeU);
		var right_pts = verb.eval.Tess.rationalCurveAdaptiveSampleRange(curve,exact_mid,end,tol,includeU);
		return left_pts.slice(0,-1).concat(right_pts);
	} else if(includeU) return [[start].concat(p1),[end].concat(p3)]; else return [p1,p3];
};
verb.eval.Tess.rationalSurfaceNaive = function(surface,divs_u,divs_v) {
	if(divs_u < 1) divs_u = 1;
	if(divs_v < 1) divs_v = 1;
	var degreeU = surface.degreeU;
	var degreeV = surface.degreeV;
	...
	var knotsU = surface.knotsU;
	var knotsV = surface.knotsV;
	var u_span = verb.core.ArrayExtensions.last(knotsU) - knotsU[0];
	var v_span = verb.core.ArrayExtensions.last(knotsV) - knotsV[0];
	var span_u = u_span / divs_u;
	var span_v = v_span / divs_v;
	var points = [];
	var uvs = [];
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			var pt_v = j * span_v;
			uvs.push([pt_u,pt_v]);
			var derivs = verb.eval.Eval.rationalSurfaceDerivatives(surface,pt_u,pt_v,1);
			var pt = derivs[0][0];
			points.push(pt);
...
verb.eval.Tess.divideRationalSurfaceAdaptive = function(surface,options) {
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			var u = umin + du * j;
			var v = vmin + dv * i;
			var ds = verb.eval.Eval.rationalSurfaceDerivatives(surface,u,v,1);
			var norm = verb.core.Vec.normalized(verb.core.Vec.cross(ds[0][1],ds[1][0]));
			ptrow.push(new verb.core.SurfacePoint(ds[0][0],norm,[u,v],-1,verb.core.Vec.isZero(norm)));
		}
		pts.push(ptrow);
	}
	var _g4 = 0;
...
verb.eval.Tess.triangulateAdaptiveRefinementNodeTree = function(arrTree) {
	var mesh = verb.core.MeshData.empty();
	var _g = 0;
...
verb.exe.Work = $hx_exports.exe.Work = function(className,methodName,args) {
			...
			worker[0].onmessage = (function(worker,workId) {
				return function(e) {
					...
					try {
						...
						haxe.Log.trace(error,{ fileName : "WorkerPool.hx", lineNumber : 78, className : "verb.exe.WorkerPool", methodName : "processQueue"});
					}
					_g.processQueue();
				};
			})(worker,workId);
...
verb.geom.NurbsCurve.byKnotsControlPointsWeights = function(degree,knots,controlPoints,weights) {
	return new verb.geom.NurbsCurve(new verb.core.NurbsCurveData(degree,knots.slice(),verb.eval.Eval.homogenize1d(controlPoints,weights)));
};
verb.geom.NurbsCurve.byPoints = function(points,degree) {
	if(degree == null) degree = 3;
	return new verb.geom.NurbsCurve(verb.eval.Make.rationalInterpCurve(points,degree));
};
...
verb.geom.NurbsCurve.prototype = $extend(verb.exe.AsyncObject.prototype,{
	...
	,domain: function() {
		return new verb.core.Interval(verb.core.ArrayExtensions.first(this._data.knots),verb.core.ArrayExtensions.last(this._data.knots));
	}
	,transform: function(mat) {
		return new verb.geom.NurbsCurve(verb.eval.Modify.rationalCurveTransform(this._data,mat));
	}
	,transformAsync: function(mat) {
		return this.defer(verb.eval.Modify,"rationalCurveTransform",[this._data,mat]).then(function(x) {
	...
	,pointAsync: function(u) {
		return this.defer(verb.eval.Eval,"rationalCurvePoint",[this._data,u]);
	}
	,tangent: function(u) {
		return verb.eval.Eval.rationalCurveTangent(this._data,u);
	}
	,tangentAsync: function(u) {
		return this.defer(verb.eval.Eval,"rationalCurveTangent",[this._data,u]);
	}
	,derivatives: function(u,numDerivs) {
		if(numDerivs == null) numDerivs = 1;
		return verb.eval.Eval.rationalCurveDerivatives(this._data,u,numDerivs);
	}
	,derivativesAsync: function(u,numDerivs) {
		if(numDerivs == null) numDerivs = 1;
		return this.defer(verb.eval.Eval,"rationalCurveDerivatives",[this._data,u,numDerivs]);
	}
	,closestPoint: function(pt) {
		return verb.eval.Analyze.rationalCurveClosestPoint(this._data,pt);
	}
	,closestPointAsync: function(pt) {
		return this.defer(verb.eval.Analyze,"rationalCurveClosestPoint",[this._data,pt]);
	}
	,closestParam: function(pt) {
		return verb.eval.Analyze.rationalCurveClosestParam(this._data,pt);
	}
	,closestParamAsync: function(pt) {
		return this.defer(verb.eval.Analyze,"rationalCurveClosestParam",[this._data,pt]);
	}
	,length: function() {
		return verb.eval.Analyze.rationalCurveArcLength(this._data);
	}
	,lengthAsync: function() {
		return this.defer(verb.eval.Analyze,"rationalCurveArcLength",[this._data]);
	}
	,lengthAtParam: function(u) {
		return verb.eval.Analyze.rationalCurveArcLength(this._data,u);
	}
	,lengthAtParamAsync: function() {
		return this.defer(verb.eval.Analyze,"rationalCurveArcLength",[this._data]);
	}
	,paramAtLength: function(len,tolerance) {
		return verb.eval.Analyze.rationalCurveParamAtArcLength(this._data,len,tolerance);
	}
	,paramAtLengthAsync: function(len,tolerance) {
		return this.defer(verb.eval.Analyze,"rationalCurveParamAtArcLength",[this._data,len,tolerance]);
	}
	...
	,divideByEqualArcLengthAsync: function(divisions) {
		return this.defer(verb.eval.Divide,"rationalCurveByEqualArcLength",[this._data,divisions]);
	}
	...
	,splitAsync: function(u) {
		return this.defer(verb.eval.Modify,"curveSplit",[this._data,u]).then(function(cs) {
	...
	,reverse: function() {
		return new verb.geom.NurbsCurve(verb.eval.Modify.curveReverse(this._data));
	}
	,reverseAsync: function() {
		return this.defer(verb.eval.Modify,"curveReverse",[this._data]).then(function(c) {
	...
	,tessellateAsync: function(tolerance) {
		return this.defer(verb.eval.Tess,"rationalCurveAdaptiveSample",[this._data,tolerance,false]);
	}
});
verb.geom.Arc = $hx_exports.geom.Arc = function(center,xaxis,yaxis,radius,minAngle,maxAngle) {
	verb.geom.NurbsCurve.call(this,verb.eval.Make.arc(center,xaxis,yaxis,radius,minAngle,maxAngle));
	this._center = center;
	this._xaxis = xaxis;
	this._yaxis = yaxis;
	this._radius = radius;
...
verb.geom.BezierCurve = $hx_exports.geom.BezierCurve = function(points,weights) {
	verb.geom.NurbsCurve.call(this,verb.eval.Make.rationalBezierCurve(points,weights));
};
...
verb.geom.NurbsSurface.byKnotsControlPointsWeights = function(degreeU,degreeV,knotsU,knotsV,controlPoints,weights) {
	return new verb.geom.NurbsSurface(new verb.core.NurbsSurfaceData(degreeU,degreeV,knotsU,knotsV,verb.eval.Eval.homogenize2d(controlPoints,weights)));
};
verb.geom.NurbsSurface.byCorners = function(point0,point1,point2,point3) {
	return new verb.geom.NurbsSurface(verb.eval.Make.fourPointSurface(point0,point1,point2,point3));
};
verb.geom.NurbsSurface.byLoftingCurves = function(curves,degreeV) {
	return new verb.geom.NurbsSurface(verb.eval.Make.loftedSurface((function($this) {
		var $r;
		var _g = [];
		{
			var _g1 = 0;
...
verb.geom.NurbsSurface.prototype = $extend(verb.exe.AsyncObject.prototype,{
	...
	,domainU: function() {
		return new verb.core.Interval(verb.core.ArrayExtensions.first(this._data.knotsU),verb.core.ArrayExtensions.last(this._data.knotsU));
	}
	,domainV: function() {
		return new verb.core.Interval(verb.core.ArrayExtensions.first(this._data.knotsV),verb.core.ArrayExtensions.last(this._data.knotsV));
	}
	,point: function(u,v) {
		return verb.eval.Eval.rationalSurfacePoint(this._data,u,v);
	}
	,pointAsync: function(u,v) {
		return this.defer(verb.eval.Eval,"rationalSurfacePoint",[this._data,u,v]);
	}
	,normal: function(u,v) {
		return verb.eval.Eval.rationalSurfaceNormal(this._data,u,v);
	}
	,normalAsync: function(u,v) {
		return this.defer(verb.eval.Eval,"rationalSurfaceNormal",[this._data,u,v]);
	}
	,derivatives: function(u,v,numDerivs) {
		if(numDerivs == null) numDerivs = 1;
		return verb.eval.Eval.rationalSurfaceDerivatives(this._data,u,v,numDerivs);
	}
	,derivativesAsync: function(u,v,numDerivs) {
		if(numDerivs == null) numDerivs = 1;
		return this.defer(verb.eval.Eval,"rationalSurfaceDerivatives",[this._data,u,v,numDerivs]);
	}
	,closestParam: function(pt) {
		return verb.eval.Analyze.rationalSurfaceClosestParam(this._data,pt);
	}
	,closestParamAsync: function(pt) {
		return this.defer(verb.eval.Analyze,"rationalSurfaceClosestParam",[this._data,pt]);
	}
	,closestPoint: function(pt) {
		return verb.eval.Analyze.rationalSurfaceClosestPoint(this._data,pt);
	}
	,closestPointAsync: function(pt) {
		return this.defer(verb.eval.Analyze,"rationalSurfaceClosestPoint",[this._data,pt]);
	}
	...
	,splitAsync: function(u,useV) {
		if(useV == null) useV = false;
		return this.defer(verb.eval.Modify,"surfaceSplit",[this._data,u,useV]).then(function(s) {
	...
	,reverse: function(useV) {
		if(useV == null) useV = false;
		return new verb.geom.NurbsSurface(verb.eval.Modify.surfaceReverse(this._data,useV));
	}
	,reverseAsync: function(useV) {
		if(useV == null) useV = false;
		return this.defer(verb.eval.Modify,"surfaceReverse",[this._data,useV]).then(function(c) {
			return new verb.geom.NurbsSurface(c);
		});
	}
	,isocurve: function(u,useV) {
		if(useV == null) useV = false;
		return new verb.geom.NurbsCurve(verb.eval.Make.surfaceIsocurve(this._data,u,useV));
	}
	,isocurveAsync: function(u,useV) {
		if(useV == null) useV = false;
		return this.defer(verb.eval.Make,"surfaceIsocurve",[this._data,u,useV]).then(function(x) {
			return new verb.geom.NurbsCurve(x);
		});
	}
	,boundaries: function(options) {
		return verb.eval.Make.surfaceBoundaryCurves(this._data).map(function(x) {
			return new verb.geom.NurbsCurve(x);
		});
	}
	,boundariesAsync: function(options) {
		return this.defer(verb.eval.Make,"surfaceBoundaryCurves",[this._data]).then(function(cs) {
			return cs.map(function(x) {
				return new verb.geom.NurbsCurve(x);
			});
		});
	...
	,tessellateAsync: function(options) {
		return this.defer(verb.eval.Tess,"rationalSurfaceAdaptive",[this._data,options]);
	}
	,transform: function(mat) {
		return new verb.geom.NurbsSurface(verb.eval.Modify.rationalSurfaceTransform(this._data,mat));
	}
	,transformAsync: function(mat) {
		return this.defer(verb.eval.Modify,"rationalSurfaceTransform",[this._data,mat]).then(function(x) {
			return new verb.geom.NurbsSurface(x);
		});
	}
});
verb.geom.ConicalSurface = $hx_exports.geom.ConicalSurface = function(axis,xaxis,base,height,radius) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.conicalSurface(axis,xaxis,base,height,radius));
	this._axis = axis;
	this._xaxis = xaxis;
	this._base = base;
	this._height = height;
...
verb.geom.CylindricalSurface = $hx_exports.geom.CylindricalSurface = function(axis,xaxis,base,height,radius) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.cylindricalSurface(axis,xaxis,base,height,radius));
	this._axis = axis;
	this._xaxis = xaxis;
	this._base = base;
	this._height = height;
...
verb.geom.EllipseArc = $hx_exports.geom.EllipseArc = function(center,xaxis,yaxis,minAngle,maxAngle) {
	verb.geom.NurbsCurve.call(this,verb.eval.Make.ellipseArc(center,xaxis,yaxis,minAngle,maxAngle));
	this._center = center;
	this._xaxis = xaxis;
	this._yaxis = yaxis;
	this._minAngle = minAngle;
...
verb.geom.ExtrudedSurface = $hx_exports.geom.ExtrudedSurface = function(profile,direction) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.extrudedSurface(verb.core.Vec.normalized(direction),verb.core.Vec.norm(direction),profile.asNurbs()));
...
verb.geom.Intersect.curvesAsync = function(first,second,tol) {
	if(tol == null) tol = 1e-3;
	return verb.exe.Dispatcher.dispatchMethod(verb.eval.Intersect,"curves",[first.asNurbs(),second.asNurbs(),tol]);
};
...
verb.geom.Intersect.curveAndSurfaceAsync = function(curve,surface,tol) {
	if(tol == null) tol = 1e-3;
	return verb.exe.Dispatcher.dispatchMethod(verb.eval.Intersect,"curveAndSurface",[curve.asNurbs(),surface.asNurbs(),tol]);
};
verb.geom.Intersect.surfaces = function(first,second,tol) {
	if(tol == null) tol = 1e-3;
	return verb.eval.Intersect.surfaces(first.asNurbs(),second.asNurbs(),tol).map(function(cd) {
		return new verb.geom.NurbsCurve(cd);
	});
};
verb.geom.Intersect.surfacesAsync = function(first,second,tol) {
	if(tol == null) tol = 1e-3;
	return verb.exe.Dispatcher.dispatchMethod(verb.eval.Intersect,"surfaces",[first.asNurbs(),second.asNurbs(),tol]).then(function(cds) {
...
verb.geom.Line = $hx_exports.geom.Line = function(start,end) {
	verb.geom.NurbsCurve.call(this,verb.eval.Make.polyline([start,end]));
	this._start = start;
	this._end = end;
};
...
verb.geom.RevolvedSurface = $hx_exports.geom.RevolvedSurface = function(profile,center,axis,angle) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.revolvedSurface(profile.asNurbs(),center,axis,angle));
	this._profile = profile;
	this._center = center;
	this._axis = axis;
	this._angle = angle;
...
verb.geom.SphericalSurface = $hx_exports.geom.SphericalSurface = function(center,radius) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.sphericalSurface(center,[0,0,1],[1,0,0],radius));
	this._center = center;
	this._radius = radius;
};
...
verb.geom.SweptSurface = $hx_exports.geom.SweptSurface = function(profile,rail) {
	verb.geom.NurbsSurface.call(this,verb.eval.Make.rationalTranslationalSurface(profile.asNurbs(),rail.asNurbs()));
	this._profile = profile;
	this._rail = rail;
};
