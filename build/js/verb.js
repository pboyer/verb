...
verb.core.MarchStepState = { __ename__ : true, __constructs__ : ["OutOfBounds","InsideDomain","AtBoundary","CompleteLoop","CoincidentStartPoint"] };
...
verb.core.MarchStepState.CoincidentStartPoint = ["CoincidentStartPoint",4];
verb.core.MarchStepState.CoincidentStartPoint.toString = $estr;
verb.core.MarchStepState.CoincidentStartPoint.__enum__ = verb.core.MarchStepState;
...
verb.core.ExpIntersect.march = function(surface0,surface1,prev,currentIndex,allStartPts,tol) {
	...
	if(prev.stepCount > 5 && verb.core.ExpIntersect.isCoincidentWithStartPoint(relaxed.point,currentIndex,allStartPts,10 * tol)) state = verb.core.MarchStepState.CoincidentStartPoint;
...
verb.core.ExpIntersect.isCoincidentWithStartPoint = function(point,currentIndex,allStartPts,tol) {
	...
	while(_g < currentIndex) {
		...
		if(dist < tol) {
			haxe.Log.trace("Coincident start point!",{ fileName : "ExpIntersect.hx", lineNumber : 253, className : "verb.core.ExpIntersect", methodName : "isCoincidentWithStartPoint"});
			return true;
		}
	}
	return false;
};
verb.core.ExpIntersect.completeMarch = function(surface0,surface1,startIndex,allStartPts,tol) {
	var start = allStartPts[startIndex];
	var step = verb.core.ExpIntersect.march(surface0,surface1,verb.core.MarchStep.init(start),startIndex,allStartPts,tol);
	if(step.state == verb.core.MarchStepState.AtBoundary || step.state == verb.core.MarchStepState.CoincidentStartPoint) return null;
	var $final = [];
	$final.push(start);
	while(step.state != verb.core.MarchStepState.CoincidentStartPoint && step.state != verb.core.MarchStepState.AtBoundary && step.state != verb.core.MarchStepState.CompleteLoop) {
		$final.push(new verb.core.types.SurfaceSurfaceIntersectionPoint(step.uv0,step.uv1,step.point,-1));
		if(step.state == verb.core.MarchStepState.CoincidentStartPoint) return null;
		step = verb.core.ExpIntersect.march(surface0,surface1,step,startIndex,allStartPts,tol);
	}
	$final.push(new verb.core.types.SurfaceSurfaceIntersectionPoint(step.uv0,step.uv1,step.point,-1));
	return $final;
};
verb.core.ExpIntersect.surfaces = function(surface0,surface1,tol) {
	var $final = [];
	var startPts = verb.core.ExpIntersect.intersectBoundaryCurves(surface0,surface1,tol);
	...
	while(_g < refinedInner.length) {
		...
		++_g;
		var res = verb.core.Intersect.curveAndSurface(verb.core.Make.surfaceIsocurve(surface0,pair.item0[0],false),surface1);
		if(res.length == 0) continue;
		if(!b) continue;
		b = false;
		var $int = new verb.core.types.SurfaceSurfaceIntersectionPoint([pair.item0[0],res[0].u],res[0].uv,res[0].curvePoint,-1);
		startPts.push($int);
	}
	var i = 0;
	while(i < startPts.length) {
		haxe.Log.trace("starting at",{ fileName : "ExpIntersect.hx", lineNumber : 326, className : "verb.core.ExpIntersect", methodName : "surfaces", customParams : [startPts[i].point]});
		var res1 = verb.core.ExpIntersect.completeMarch(surface0,surface1,i,startPts,tol);
		if(res1 != null) {
			$final.push(res1);
			startPts.splice(i,0,res1[res1.length - 1]);
			haxe.Log.trace("inserting",{ fileName : "ExpIntersect.hx", lineNumber : 334, className : "verb.core.ExpIntersect", methodName : "surfaces", customParams : [res1[res1.length - 1].point]});
			i += 2;
		}
		i++;
	}
	...
	while(_g11 < $final.length) {
		var pts = $final[_g11];
		++_g11;
		_g1.push(verb.core.Make.rationalInterpCurve(pts.map(function(x) {
			return x.point;
		})));
	}
	return _g1;
...
verb.core.Intersect.boundingBoxTrees = function(ai,bi,tol) {
	...
	while(atrees.length > 0) {
		...
		if(ai1 && bi1) {
			results.push(new verb.core.types.Pair(a["yield"](),b["yield"]()));
			continue;
		} else if(ai1 && !bi1) {
			var bs = b.split();
			atrees.push(a);
...
verb.topo.Boolean.connect = function(nesa,nesb,afaces,bfaces) {
	...
	while(_g1 < _g) {
		...
		if((h0 = verb.topo.Boolean.canJoin(nea,neb.opp,looseendsa,looseendsb)) != null) {
			haxe.Log.trace("joining 1!",{ fileName : "Boolean.hx", lineNumber : 165, className : "verb.topo.Boolean", methodName : "connect"});
			var h0a = h0.item0;
			var h0b = h0.item1;
			haxe.Log.trace("connecting:",{ fileName : "Boolean.hx", lineNumber : 170, className : "verb.topo.Boolean", methodName : "connect", customParams : [h0a.id,nea.id]});
		...
		if((h1 = verb.topo.Boolean.canJoin(nea.opp,neb,looseendsa,looseendsb)) != null) {
			haxe.Log.trace("joining 2!",{ fileName : "Boolean.hx", lineNumber : 186, className : "verb.topo.Boolean", methodName : "connect"});
			var h1a = h1.item0;
			var h1b = h1.item1;
			haxe.Log.trace("connecting:",{ fileName : "Boolean.hx", lineNumber : 191, className : "verb.topo.Boolean", methodName : "connect", customParams : [h1a.id,h1b.id,neb.id,nea.opp.id]});
			verb.topo.Split.join(h1a,nea.opp);
			if(!verb.topo.Split.isLoose(h1a.opp,looseendsa)) verb.topo.Split.cut(h1a,afaces);
			haxe.Log.trace("alright",{ fileName : "Boolean.hx", lineNumber : 198, className : "verb.topo.Boolean", methodName : "connect"});
			verb.topo.Split.join(h1b,neb);
			if(!verb.topo.Split.isLoose(h1b.opp,looseendsb)) verb.topo.Split.cut(h1b,bfaces);
		}
		if(h0 != null && h1 != null) {
			haxe.Log.trace("cutting",{ fileName : "Boolean.hx", lineNumber : 206, className : "verb.topo.Boolean", methodName : "connect"});
...
verb.topo.Split.join = function(e0,e1) {
	...
	if(e0.nxt.nxt != e1) {
		s.lmef(e1,e0.nxt);
		if(nf != null && of.l.nxt != of.l) haxe.Log.trace("PANIC!",{ fileName : "Split.hx", lineNumber : 285, className : "verb.topo.Split", methodName : "join"});
	}
};
