...
{
	var nurbs = module.exports
		, numeric = require('numeric')
		, binomial = require('./binomial');
}

(function( numeric, nurbs, binomial, undefined ) {
	...
	nurbs.rational_surface_derivs = function( degree_u, knot_vector_u, degree_v, knot_vector_v, homo_control_points, num_derivs, u, v) {

		var SKL_homo = nurbs.surface_derivs( degree_u, knot_vector_u, degree_v, knot_vector_v, homo_control_points, num_derivs, u, v )
			, ders = nurbs.separate_homo_derivs_2d( SKL_homo )
			, Aders = ders[0]
			, wders = ders[1]
			, k = 0
			, i  = 0
	...
	nurbs.rational_surface_point = function( degree_u, knot_vector_u,  degree_v, knot_vector_v, homo_control_points, u, v ) {

		return nurbs.dehomogenize( nurbs.surface_point( degree_u, knot_vector_u,  degree_v, knot_vector_v, homo_control_points, u, v ) );
	...
	nurbs.rational_curve_derivs = function( degree, knot_vector, homo_control_points, u, num_derivs ) {

		// compute the derivatives of the control points
		// separate derivative array into two
		var ders = nurbs.separate_homo_derivs_1d( nurbs.curve_derivs( degree, knot_vector, homo_control_points, u, num_derivs ) )
			, Aders = ders[0]
			, wders = ders[1]
			, k = 0
			, i  = 0
	...
	nurbs.separate_homo_derivs_1d = function( CK ) {
	...
	nurbs.separate_homo_derivs_2d = function( SKL ) {

		var Aders = []
			, wders = [];

		for ( var i = 0, l = SKL.length; i < l; i++ ) {
			var CK = nurbs.separate_homo_derivs_1d( SKL[i] );
			Aders.push( CK[0] );
			wders.push( CK[1] );
		}
	...
	nurbs.rational_curve_point = function( degree, knot_vector, homo_control_points, u) {

		return nurbs.dehomogenize( nurbs.curve_point( degree, knot_vector, homo_control_points, u) );
	...
	nurbs.dehomogenize = function( homo_point ) {
	...
	nurbs.homogenize_1d = function( control_points, weights) {
	...
	nurbs.homogenize_2d = function( control_points, weights) {
		...
		for (var i = 0; i < rows; i++) {
			homo_control_points.push( nurbs.homogenize_1d(control_points[i], weights[i]) );
		}
	...
	nurbs.surface_derivs = function( degree_u, knot_vector_u, degree_v, knot_vector_v, control_points, num_derivatives, u, v ) {

		var n = knot_vector_u.length - degree_u - 2
			, m = knot_vector_v.length - degree_v - 2;

		return nurbs.surface_derivs_given_n_m( n, degree_u, knot_vector_u, m, degree_v, knot_vector_v, control_points, num_derivatives, u, v );
	...
	nurbs.surface_derivs_given_n_m = function( n, degree_u, knot_vector_u, m, degree_v, knot_vector_v, control_points, num_derivatives, u, v ) {

		if ( nurbs.are_valid_relations(degree_u, control_points.length, knot_vector_u.length ) === false ||
			nurbs.are_valid_relations(degree_v, control_points[0].length, knot_vector_v.length ) === false ) {
		...
		var dim = control_points[0][0].length
			...
			, SKL = nurbs.zeros_3d( du+1, dv+1, dim )
			, knot_span_index_u = nurbs.knot_span_given_n( n, degree_u, u, knot_vector_u )
			, knot_span_index_v = nurbs.knot_span_given_n( m, degree_v, v, knot_vector_v )
			, uders = nurbs.deriv_basis_functions_given_n_i( knot_span_index_u, u, degree_u, n, knot_vector_u )  
			, vders = nurbs.deriv_basis_functions_given_n_i( knot_span_index_v, v, degree_v, m, knot_vector_v )
			, temp = nurbs.zeros_2d( degree_v+1, dim )
			, k = 0
			, s = 0
			, r = 0
			, l = 0
	...
	nurbs.surface_point = function( degree_u, knot_vector_u,  degree_v, knot_vector_v, control_points, u, v ) {

		var n = knot_vector_u.length - degree_u - 2
			, m = knot_vector_v.length - degree_v - 2;

		return 	nurbs.surface_point_given_n_m( n, degree_u, knot_vector_u, m, degree_v, knot_vector_v, control_points, u, v );
	...
	nurbs.surface_point_given_n_m = function( n, degree_u, knot_vector_u, m, degree_v, knot_vector_v, control_points, u, v ) {
		...
		var dim = control_points[0][0].length
			, knot_span_index_u = nurbs.knot_span_given_n( n, degree_u, u, knot_vector_u )
			, knot_span_index_v = nurbs.knot_span_given_n( m, degree_v, v, knot_vector_v )
			, u_basis_vals = nurbs.basis_functions_given_knot_span_index( knot_span_index_u, u, degree_u, knot_vector_u )
			, v_basis_vals = nurbs.basis_functions_given_knot_span_index( knot_span_index_v, v, degree_v, knot_vector_v )
			, uind = knot_span_index_u - degree_u
			, vind = knot_span_index_v
			, position = nurbs.zeros_1d( control_points[0][0].length )
			, temp = nurbs.zeros_1d( control_points[0][0].length )
			, l = 0
			, k = 0;
	...
	nurbs.curve_derivs = function( degree, knot_vector, control_points, u, num_derivs ) {

		var n = knot_vector.length - degree - 2;
		return nurbs.curve_derivs_given_n( n, degree, knot_vector, control_points, u, num_derivs );
	...
	nurbs.curve_derivs_given_n = function( n, degree, knot_vector, control_points, u, num_derivatives ) {

		if ( nurbs.are_valid_relations(degree, control_points.length, knot_vector.length ) === false ) {
			console.error('Invalid relations between control points, knot vector, and n');
			return null;
		}
		...
		var dim = control_points[0].length
			...
			, CK = nurbs.zeros_2d( du+1, dim )
			, knot_span_index = nurbs.knot_span_given_n( n, degree, u, knot_vector )
			, nders = nurbs.deriv_basis_functions_given_n_i( knot_span_index, u, degree, du, knot_vector )
			, k = 0
			, j = 0;
	...
	nurbs.are_valid_relations = function( degree, num_control_points, knot_vector_length ) {
	...
	nurbs.curve_point = function( degree, knot_vector, control_points, u) {

		var n = knot_vector.length - degree - 2;
		return nurbs.curve_point_given_n( n, degree, knot_vector, control_points, u);
	...
	nurbs.curve_point_given_n = function( n, degree, knot_vector, control_points, u) {
		...
		var knot_span_index = nurbs.knot_span_given_n( n, degree, u, knot_vector )
			, basis_values = nurbs.basis_functions_given_knot_span_index( knot_span_index, u, degree, knot_vector ) 
			, position = nurbs.zeros_1d( control_points[0].length );
	...
	nurbs.zeros_2d = function(rows, cols) {
	  cols = cols > 0 ? cols : 0;
	  rows = rows > 0 ? rows : 0;
	...
	nurbs.zeros_3d = function(rows, cols, dim) {
	  cols = cols > 0 ? cols : 0;
	  rows = rows > 0 ? rows : 0;
	  ...
	  while(rows--) {
	    ...
	    while(cols_temp--) {
	      arr[rows_temp-rows-1].push( nurbs.zeros_1d(dim) );
	    }
	    cols_temp = cols;
	  }
	...
	nurbs.deriv_basis_functions = function( u, degree, knot_vector )
	{
		var knot_span_index = nurbs.knot_span( degree, u, knot_vector )
			, m = knot_vector.length - 1
			, n = m - degree - 1;

		return nurbs.deriv_basis_functions_given_n_i( knot_span_index, u, degree, n, knot_vector );
	}	
	...
	nurbs.deriv_basis_functions_given_n_i = function( knot_span_index, u, p, n, knot_vector )
	{
		var ndu = nurbs.zeros_2d(p+1, p+1)
			, left = new Array( p + 1 )
			, right = new Array( p + 1 )
			, saved = 0
			, temp = 0
		...
		var ders = nurbs.zeros_2d(n+1, p+1)
			, a = nurbs.zeros_2d(2, p+1)
			, k = 1
			, s1 = 0
			, s2 = 1
			, d = 0
	...
	nurbs.basis_functions = function( u, degree, knot_vector )
	{
		var knot_span_index = nurbs.knot_span(u, degree, knot_vector);
		return nurbs.basis_functions_given_knot_span_index( knot_span_index, u, degree, knot_vector );
	};
	...
	nurbs.basis_functions_given_knot_span_index = function( knot_span_index, u, degree, knot_vector )
	{
	...
	nurbs.knot_span = function( degree, u, knot_vector )
	{

		var m = knot_vector.length - 1
			, n = m - degree - 1;

		return nurbs.knot_span_given_n(n, degree, u, knot_vector);
	...
	nurbs.knot_span_given_n = function( n, degree, u, knot_vector )
	{
